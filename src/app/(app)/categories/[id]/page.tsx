...
export default async function Page({
  ...
  const [category, products]: [CategoryResponse, ProductsResponse] =
    await Promise.all([
      (await fetch(`${process.env.API_URL}/api/v1/categories/${id}`)).json(),
      (
        await fetch(
          `${process.env.API_URL}/api/v1/products?category=${id}&page=${pageNumber}&limit=12`,
        )
      ).json(),
    ]);
  return (
    <main className="container p-5 space-y-5 mx-auto min-h-[calc(100vh-65px)] flex justify-between flex-col">
      <section className="space-y-5">
        <section className="py-5 space-y-5">
          ...
          <p>
            Discover all {category.data.name} products available in our shop.
          </p>
        </section>
          ...
          {products.data.length === 0 ? (
            <div className="col-span-full gap-5 h-[50vh] flex flex-col items-center justify-center py-5">
              ...
              <p className="text-muted-foreground text-2xl">
                No products found under {category.data.name} yet
              </p>
            </div>
          ) : (
